...
typedef struct
{
	ID3D11Buffer	*g_ertexBuffer;	// 頂点バッファ
						...
						int nNumBlockX, int nNumBlockY, float fBlockSizeX, float fBlockSizeZ)
{
	...
	GetDevice()->CreateBuffer(&bd, NULL, &pMesh->g_ertexBuffer);
	...
	{//頂点バッファの中身を埋める
		...
		GetDeviceContext()->Map(pMesh->g_ertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
		...
		GetDeviceContext()->Unmap(pMesh->g_ertexBuffer, 0);
	}
...
void UninitMeshWall(void)
{
	...
	for(nCntMeshField = 0; nCntMeshField < g_nNumMeshWall; nCntMeshField++)
	{
		...
		if(pMesh->g_ertexBuffer)
		{// 頂点バッファの解放
			pMesh->g_ertexBuffer->Release();
			pMesh->g_ertexBuffer = NULL;
		}
...
void DrawMeshWall(void)
{
	...
	for(nCntMeshField = 0; nCntMeshField < g_nNumMeshWall; nCntMeshField++)
	{
		...
		UINT offset = 0;
		GetDeviceContext()->IASetVertexBuffers(0, 1, &pMesh->g_ertexBuffer, &stride, &offset);
