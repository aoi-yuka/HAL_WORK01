...
#define	SHADOW_OFFSET		(7.0f)			// 影の高さを変えない
...
void DrawShadow(void)
{
	...
	for(int i = 0; i < MAX_SHADOW; i++)
	{
		if(g_aShadow[i].bUse)
		{
			...
			// 移動を反映
			mtxTranslate = XMMatrixTranslation(g_aShadow[i].pos.x, SHADOW_OFFSET, g_aShadow[i].pos.z);
...
HRESULT MakeVertexShadow()
{
	...
	{//頂点バッファの中身を埋める
		...
		GetDeviceContext()->Map(g_ertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
		...
		GetDeviceContext()->Unmap(g_ertexBuffer, 0);
	}
